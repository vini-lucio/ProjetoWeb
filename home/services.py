...
from home.models import (Cidades, Unidades, Produtos, Estados, EstadosIcms, Vendedores, CanaisVendas, Regioes,
                         ProdutosModelos, ProdutosTipos, ProdutosFamilias)
...
from analysis.models import (VENDEDORES, ESTADOS, MATRIZ_ICMS, FAIXAS_CEP, UNIDADES, PRODUTOS, CANAIS_VENDA, REGIOES,
                             TIPO_PRODUTOS, APONTAMENTOS_PARADAS, FAMILIA_PRODUTOS)
...
def forma_captacao_clientes_ano_mes_a_mes():
    ...
    sql = """
        SELECT
            FORMAS_CAPTACAO.DESCRICAO AS FORMAS_CAPTACAO,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano - 1 THEN CLIENTES.CODCLI END) AS QUANTIDADE_ANTERIOR,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano THEN CLIENTES.CODCLI END) AS QUANTIDADE_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 1 THEN CLIENTES.CODCLI END) AS JAN_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 2 THEN CLIENTES.CODCLI END) AS FEV_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 3 THEN CLIENTES.CODCLI END) AS MAR_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 4 THEN CLIENTES.CODCLI END) AS ABR_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 5 THEN CLIENTES.CODCLI END) AS MAI_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 6 THEN CLIENTES.CODCLI END) AS JUN_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 7 THEN CLIENTES.CODCLI END) AS JUL_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 8 THEN CLIENTES.CODCLI END) AS AGO_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 9 THEN CLIENTES.CODCLI END) AS SET_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 10 THEN CLIENTES.CODCLI END) AS OUT_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 11 THEN CLIENTES.CODCLI END) AS NOV_ATUAL,
            COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = :ano AND EXTRACT(MONTH FROM TO_DATE(CLIENTES.LOG_DATA_INCLUSAO, 'DD-MM-YYYY, HH24:MI')) = 12 THEN CLIENTES.CODCLI END) AS DEZ_ATUAL

        FROM
            COPLAS.FORMAS_CAPTACAO,
            COPLAS.CLIENTES

        WHERE
            CLIENTES.CHAVE_FORMA_CAPTACAO = FORMAS_CAPTACAO.CHAVE AND
            FORMAS_CAPTACAO.DESCRICAO NOT LIKE 'ZZZ%' AND
            (
                CLIENTES.LOG_DATA_INCLUSAO LIKE '%{ano_anterior}%' OR
                CLIENTES.LOG_DATA_INCLUSAO LIKE '%{ano}%'
            )

        GROUP BY
            FORMAS_CAPTACAO.DESCRICAO

        ORDER BY
            FORMAS_CAPTACAO.DESCRICAO
    """

    sql = sql.format(ano_anterior=ano - 1, ano=ano)
        ...
        FROM
            (
                SELECT
                    ULTIMA_MOVIMENTACAO.MES,
                    PRODUTOS.CPROD,
                    PRODUTOS_JOBS_CUSTOS.PRECO_ICMS18 AS TABELA,
...
def migrar_produtos_familias():
    mapeamento_destino_origem = {
        'descricao': 'FAMILIA',
    }

    origem = FAMILIA_PRODUTOS.objects.all()
    if origem:
        destino = ProdutosFamilias.objects
        for objeto_origem in origem:
            objeto_destino = destino.filter(chave_analysis=objeto_origem.pk).first()
            ...
            if mudou:
                instancia, criado = destino.update_or_create(
                    ...
                    defaults={
                        'chave_analysis': objeto_origem.pk,
                        'descricao': objeto_origem.FAMILIA,
                    }
                )
                instancia.full_clean()
                instancia.save()
...
def migrar_produtos():
    mapeamento_destino_origem = {
        ...
        'familia': ('CHAVE_FAMILIA', ('chave_analysis', 'CHAVE')),
    }
    ...
    if origem:
        ...
        familia = ProdutosFamilias.objects
        for objeto_origem in origem:
            objeto_destino = destino.filter(chave_analysis=objeto_origem.pk).first()
            ...
            if mudou:
                ...
                fk_familia = familia.filter(chave_analysis=objeto_origem.CHAVE_FAMILIA.CHAVE).first()  # type:ignore
