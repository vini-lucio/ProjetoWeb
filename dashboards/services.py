...
def map_relatorio_vendas_sql_string_placeholders(fonte: Literal['orcamentos', 'pedidos', 'faturamentos'],
    ...
    map_sql_notas = {
        ...
        'coluna_aliquotas_itens': {'lfrete_coluna_aliquotas_itens': notas_lfrete_aliquotas_itens_coluna,
                                   'lfrete_from': notas_lfrete_from,
                                   'lfrete_join': notas_lfrete_join, },
        'coluna_mc_cor_ajuste': {'mc_cor_ajuste_campo_alias': ", CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7766 THEN (-1) WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN 4 WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN 4 WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN 2 END AS MC_COR_AJUSTE",
                                 'mc_cor_ajuste_campo': "CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7766 THEN (-1) WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN 4 WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN 4 WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN 2 END,", },
    ...
    pedidos_lfrete_cor_coluna = """
        ...
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PT {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.04) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
    ...
    orcamentos_lfrete_cor_coluna = """
        ...
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PT {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.04) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
