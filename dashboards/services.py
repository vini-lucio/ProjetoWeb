            ...
            (
                SELECT DISTINCT
                    ...
                    CASE
                        ...
                        WHEN ORCAMENTOS.CHAVE_JOB != 25 AND REPRESENTANTES.NOME LIKE '%FLUXUS%' THEN 'REPRESENTANTE INCORRETO'
                        WHEN ORCAMENTOS.CHAVE_TIPO = 43 AND PRODUTOS.CHAVE_LINHA = 11299 THEN 'TIPO DE PEDIDO INCORRETO (PROTECAO)'
                        WHEN ORCAMENTOS.CHAVE_JOB = 22 AND ORCAMENTOS.CHAVE_TRANSPORTADORA = 6766 AND ORCAMENTOS.COBRANCA_FRETE NOT IN (4, 5) THEN 'PARA TRANSPORTADORA COPLAS TIPO DE FRETE DEVE SER "TRANSPORTE PROPRIO COM / SEM COBRANCA DE FRETE"'
                        WHEN ORCAMENTOS.CHAVE_JOB = 22 AND ORCAMENTOS.CHAVE_TRANSPORTADORA != 6766 AND ORCAMENTOS.COBRANCA_FRETE IN (4, 5) THEN 'PARA TRANSPORTADORAS TIPO DE FRETE NAO DEVE SER "TRANSPORTE PROPRIO COM / SEM COBRANCA DE FRETE"'
                ...
                WHERE
                    ...
                    REPRESENTANTES.CODVENDEDOR = ORCAMENTOS.CHAVE_VENDEDOR AND
            ...
            (
                SELECT DISTINCT
                    ...
                    CASE
                        ...
                        WHEN PEDIDOS.CHAVE_JOB = 25 AND (PEDIDOS.CHAVE_VENDEDOR != 640 AND REPRESENTANTES.NOME NOT LIKE '%FLUXUS%') THEN 'REPRESENTANTE INCORRETO'
                        WHEN PEDIDOS.CHAVE_JOB != 25 AND REPRESENTANTES.NOME LIKE '%FLUXUS%' THEN 'REPRESENTANTE INCORRETO'
                        WHEN PEDIDOS.CHAVE_TIPO = 43 AND PRODUTOS.CHAVE_LINHA = 11299 THEN 'TIPO DE PEDIDO INCORRETO (PROTECAO)'
                        WHEN PEDIDOS.CHAVE_JOB = 22 AND PEDIDOS.CHAVE_TRANSPORTADORA = 6766 AND PEDIDOS.COBRANCA_FRETE NOT IN (4, 5) THEN 'PARA TRANSPORTADORA COPLAS TIPO DE FRETE DEVE SER "TRANSPORTE PROPRIO COM / SEM COBRANCA DE FRETE"'
                        WHEN PEDIDOS.CHAVE_JOB = 22 AND PEDIDOS.CHAVE_TRANSPORTADORA != 6766 AND PEDIDOS.COBRANCA_FRETE IN (4, 5) THEN 'PARA TRANSPORTADORAS TIPO DE FRETE NAO DEVE SER "TRANSPORTE PROPRIO COM / SEM COBRANCA DE FRETE"'
                ...
                WHERE
                    ...
                    CLIENTES.CODCLI = CLIENTES_EMAILS_BOLETOS.CHAVE_CLIENTE(+) AND
                    VENDEDORES.CODVENDEDOR = CLIENTES.CHAVE_VENDEDOR3 AND
                    REPRESENTANTES.CODVENDEDOR = PEDIDOS.CHAVE_VENDEDOR AND
...
def map_relatorio_vendas_sql_string_placeholders(fonte: Literal['orcamentos', 'pedidos', 'faturamentos'],
    ...
    notas_lfrete_cor_coluna = """
        ...
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PT) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN NOTAS_ITENS.VALOR_MERCADORIAS - (NOTAS_ITENS.PESO_LIQUIDO / NOTAS_PESO_LIQUIDO.PESO_LIQUIDO * NOTAS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN NOTAS_ITENS.VALOR_MERCADORIAS - (NOTAS_ITENS.PESO_LIQUIDO / NOTAS_PESO_LIQUIDO.PESO_LIQUIDO * NOTAS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PQ) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN NOTAS_ITENS.VALOR_MERCADORIAS - (NOTAS_ITENS.PESO_LIQUIDO / NOTAS_PESO_LIQUIDO.PESO_LIQUIDO * NOTAS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN NOTAS_ITENS.VALOR_MERCADORIAS - (NOTAS_ITENS.PESO_LIQUIDO / NOTAS_PESO_LIQUIDO.PESO_LIQUIDO * NOTAS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN NOTAS_ITENS.VALOR_MERCADORIAS - (NOTAS_ITENS.PESO_LIQUIDO / NOTAS_PESO_LIQUIDO.PESO_LIQUIDO * NOTAS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.02) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN NOTAS_ITENS.VALOR_MERCADORIAS - (NOTAS_ITENS.PESO_LIQUIDO / NOTAS_PESO_LIQUIDO.PESO_LIQUIDO * NOTAS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS_PT) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN NOTAS_ITENS.VALOR_MERCADORIAS - (NOTAS_ITENS.PESO_LIQUIDO / NOTAS_PESO_LIQUIDO.PESO_LIQUIDO * NOTAS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN NOTAS_ITENS.VALOR_MERCADORIAS - (NOTAS_ITENS.PESO_LIQUIDO / NOTAS_PESO_LIQUIDO.PESO_LIQUIDO * NOTAS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
    ...
    notas_lfrete_from = """
        (
            SELECT
                ...
                ROUND(SUM(CASE WHEN CHAVE_FAMILIA = 7767 THEN MC + PIS + COFINS + ICMS + IR + CSLL + COMISSAO_FRETE_ITEM + DESPESA_ADM_FRETE_ITEM + DESPESA_COM_FRETE_ITEM ELSE 0 END), 2) AS MC_SEM_FRETE_PT,
                ...
                ROUND(SUM(CASE WHEN CHAVE_FAMILIA = 12441 THEN MC + PIS + COFINS + ICMS + IR + CSLL + COMISSAO_FRETE_ITEM + DESPESA_ADM_FRETE_ITEM + DESPESA_COM_FRETE_ITEM ELSE 0 END), 2) AS MC_SEM_FRETE_FLUXUS,
                ROUND(SUM(CASE WHEN CHAVE_FAMILIA = 12819 THEN MC + PIS + COFINS + ICMS + IR + CSLL + COMISSAO_FRETE_ITEM + DESPESA_ADM_FRETE_ITEM + DESPESA_COM_FRETE_ITEM ELSE 0 END), 2) AS MC_SEM_FRETE_FLUXUS_PT,
    ...
    pedidos_lfrete_cor_coluna = """
        ...
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PT) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PQ) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.02) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS_PT) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        ...
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PT {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PQ {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.02) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS_PT {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN PEDIDOS_ITENS.VALOR_TOTAL - (PEDIDOS_ITENS.PESO_LIQUIDO / PEDIDOS.PESO_LIQUIDO * PEDIDOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
        , 0), 2) AS MC_VALOR_COR
    ...
    orcamentos_lfrete_cor_coluna = """
        ...
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PT) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PQ) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.02) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS_PT) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END), 0), 0)
        ...
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PT {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 7767 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_PQ {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 8378 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.02) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12441 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
        + (COALESCE(SUM(LFRETE.MC_SEM_FRETE_FLUXUS_PT {conversao_moeda}) / NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0) + 0.05) * COALESCE(NULLIF(SUM(CASE WHEN PRODUTOS.CHAVE_FAMILIA = 12819 THEN ORCAMENTOS_ITENS.VALOR_TOTAL - (ORCAMENTOS_ITENS.PESO_LIQUIDO / ORCAMENTOS.PESO_LIQUIDO * ORCAMENTOS.VALOR_FRETE_INCL_ITEM) ELSE 0 END {conversao_moeda}), 0), 0)
        , 0), 2) AS MC_VALOR_COR
    """.format(conversao_moeda=conversao_moeda)
